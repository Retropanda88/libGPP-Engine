...
SDL_Surface* Sprite::getCurrentFrame() {
    if (!sprite || maxFrames == 0) return NULL;

    SDL_Surface* frameSurface = SDL_CreateRGBSurface(
        SDL_HWSURFACE, width, height,
    ...
    SDL_Rect srcRect;
    srcRect.x = index * width;
    srcRect.y = 0;
    srcRect.w = (Uint16)width;
    srcRect.h = (Uint16)height;
    ...
    SDL_BlitSurface(sprite, &srcRect, frameSurface, &destRect);
...
Sprite::Sprite(u8* buffer, int len, int frames, int speed) {
    sprite = NULL;
    loaded = false;
    run = false;
    index = 0;
    ...
    SDL_RWops* mem_rwops = SDL_RWFromMem(buffer, len);
    if (!mem_rwops) {
    ...
    if (sprite) {
        width = sprite->w / maxFrames; // ancho por frame
        height = sprite->h;            // alto total
        loaded = true;
    }
}

void Sprite::load(const char *file, int frames, int speed) {
