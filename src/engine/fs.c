...
/* abrir archivo */
/* ---------------------------------------------------- */
/* FS_FILE* fs_open(const char *path, const char *mode) { FS_FILE *f; const
   char *full;

   if (!path) return 0;

   f = (FS_FILE*)malloc(sizeof(FS_FILE)); if (!f) return 0;

   full = fs_make_path(path);

   #if defined(PS2_BUILD) f->handle = (void*)fioOpen(full, O_RDONLY);

   #elif defined(PSP_BUILD) f->handle = (void*)sceIoOpen(full, PSP_O_RDONLY,
   0777);

   #else f->handle = fopen(full, mode); #endif
...
/* ---------------------------------------------------- */
/* int fs_read(void *ptr, u32 size, u32 count, FS_FILE *stream) { u32 bytes;
   int r = 0;
   ...
   #if defined(PS2_BUILD) r = fioRead((int)stream->handle, ptr, bytes);

   #elif defined(PSP_BUILD) r = sceIoRead((SceUID)stream->handle, ptr, bytes);

   #else r = fread(ptr, 1, bytes, (FILE*)stream->handle); #endif

   if (r > 0) stream->pos += (u32)r;
...
/* ---------------------------------------------------- */
/* int fs_seek(FS_FILE *stream, s32 offset, int origin) { if (!stream) return
   -1;

   #if defined(PS2_BUILD) stream->pos = (u32)fioLseek((int)stream->handle,
   offset, origin);

   #elif defined(PSP_BUILD) stream->pos =
   (u32)sceIoLseek((SceUID)stream->handle, offset, origin);

   #else fseek((FILE*)stream->handle, offset, origin); stream->pos =
   (u32)ftell((FILE*)stream->handle); #endif
...
/* cerrar archivo */
/* ---------------------------------------------------- */
/* void fs_close(FS_FILE *stream) { if (!stream) return;

   #if defined(PS2_BUILD) fioClose((int)stream->handle);

   #elif defined(PSP_BUILD) sceIoClose((SceUID)stream->handle);

   #else fclose((FILE*)stream->handle); #endif
