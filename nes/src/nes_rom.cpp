...
findAlternateExtInZip(unzFile file, const char *path, const char *ext)
{
  ...
  name = (char *)malloc(strlen(path) + strlen(ext) + 1);
  strcpy(name, p);
...
#ifdef ENABLE_UNZIP
    if (isZip)
      image_type = load_nes_rom ((FILE *)zipfp, isZip);
    else
#endif
...
#ifdef ENABLE_UNZIP
    if (isZip)
      image_type = load_fds_rom ((FILE *)zipfp, isZip);
    else
#endif
...
NES_ROM::load_nes_rom (FILE *fp, bool isZip)
{
  size_t nread;
...
NES_ROM::rom_image_type
NES_ROM::load_fds_rom (FILE *fp, bool isZip)
{
  int i;
...
#include "nes_rom_correct.cpp"
  ...
  // PARCHE DE EMERGENCIA PARA CONTROLLER/MAPPER:
  // Forzamos siempre la lectura de los bits altos del mapper (flags_2).
  // Las ROMs modernas de NES suelen traer datos en la cabecera ($8-$F) que este
  // emulador viejo interpretaba erróneamente como corruptas, rompiendo el scroll.
  mapper |= (header.flags_2 & 0xF0);

  // Dejamos el bucle solo para fines informativos en el log sin alterar el flujo del juego
  ...
  if (!header_valid) 
  {
    LOG("Modern/Modified iNES header detected ($8-$F) - Bypassed safely");
  }
}
