...
void NES_mapper4::Reset()
{
  ...
  if(parent_NES->crc32() == 0xdebea5a6 || // Ninja Ryukenden 2 - Ankoku no Jashin Ken
     parent_NES->crc32() == 0xc43da8e2 || // Ninja Gaiden 2
...
void NES_mapper4::HSync(uint32 scanline)
{
  if(irq_enabled)
  {
    ...
    {
      if (attached_ppu->bg_or_spr_enabled())
      {
        if(patch == 1)
        {
          if(!(--irq_counter))
...
#define MAP4_VRAM(ptr) (((ptr) - attached_ppu->get_patt()) >> 10)
...
void NES_mapper4::SNSS_fixup() // HACK HACK HACK HACK
{
  ...
  if(num_1k_VROM_banks)
  {
    if(chr_swap())
    {
      chr01 = MAP4_VROM(attached_ppu->PPU_VRAM_banks[4]);
      chr23 = MAP4_VROM(attached_ppu->PPU_VRAM_banks[6]);
      chr4  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[0]);
      chr5  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[1]);
      chr6  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[2]);
      chr7  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[3]);
    }
    else
    {
      chr01 = MAP4_VROM(attached_ppu->PPU_VRAM_banks[0]);
      chr23 = MAP4_VROM(attached_ppu->PPU_VRAM_banks[2]);
      chr4  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[4]);
      chr5  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[5]);
      chr6  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[6]);
      chr7  = MAP4_VROM(attached_ppu->PPU_VRAM_banks[7]);
    }
  }
  else
  {
    if(chr_swap())
    {
      chr01 = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[4]);
      chr23 = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[6]);
      chr4  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[0]);
      chr5  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[1]);
      chr6  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[2]);
      chr7  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[3]);
    }
    else
    {
      chr01 = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[0]);
      chr23 = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[2]);
      chr4  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[4]);
      chr5  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[5]);
      chr6  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[6]);
      chr7  = MAP4_VRAM(attached_ppu->PPU_VRAM_banks[7]);
    }
  }
}
/////////////////////////////////////////////////////////////////////
