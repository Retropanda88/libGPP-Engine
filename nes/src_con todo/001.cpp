...
void NES_mapper1::Reset()
{
  ...
  // Cálculo directo de páginas fijas superiores
  MMC1_HI1 = num_8k_ROM_banks - 2;
  MMC1_HI2 = num_8k_ROM_banks - 1;

  // Punteros iniciales de la CPU
  MMC1_bank1 = 0;
  MMC1_bank2 = 1;
  MMC1_bank3 = MMC1_HI1;
  MMC1_bank4 = MMC1_HI2;
...
void NES_mapper1::MMC1_set_CPU_banks()
{
  if (MMC1_Size == MMC1_SMALL)
  {
    set_CPU_banks(MMC1_bank1, MMC1_bank2, MMC1_bank3, MMC1_bank4);
  }
  else
  {
    set_CPU_banks((MMC1_256K_base << 5) + (MMC1_bank1 & 31),
                  (MMC1_256K_base << 5) + (MMC1_bank2 & 31),
                  (MMC1_256K_base << 5) + (MMC1_bank3 & 31),
                  (MMC1_256K_base << 5) + (MMC1_bank4 & 31));
  }
}
...
void NES_mapper1::MemoryWrite(uint32 addr, uint8 data)
{
  ...
  // Lógica oficial de Reset (Bit 7) de la NES
  if(data & 0x80)
  {
    write_count = 0;
    bits = 0x00;
    ...
    case 0:
      {
        ...
        // PARCHE DE EMERGENCIA DE SINCRONIZACIÓN PARA PS2:
        // Cuando el juego reconfigura el registro espejo o scroll (esencial en las intros),
        // obligamos al emulador a recalcular el contexto de ejecución. Esto rompe los bucles 
        // infinitos de espera de ciclos de la CPU MIPS de PS2 en instrucciones estancadas.
        nes6502_setcontext();
      }
      break;
