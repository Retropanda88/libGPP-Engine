...
int main(int argc, char **argv)
{
    ...
    // Configuración de volúmenes solicitada:
    // - Maestro al 100% (MAX_VOLUME)
    // - Música al 70%
    int masterVol = MAX_VOLUME;
    int musicVol = (MAX_VOLUME * 70) / 100;
    int sfxVol = (MAX_VOLUME * 90) / 100; // 90% para los efectos (samples)

    mixer.setMasterVolume(masterVol);
    mixer.setMusicVolume(musicVol);
    ...
    while (!isDone)
    {
        ...
        if (moveDown)
        {
            ...
            if (sndMove.getData() != NULL) {
                mixer.playChannel(&sndMove, false, sfxVol, -1);
            }
        }
        if (moveUp)
        {
        ...
        if (press_a)
        {
            if (sndSelect.getData() != NULL) {
                mixer.playChannel(&sndSelect, false, sfxVol, -1);
            }
        }
